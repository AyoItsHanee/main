...
game:GetService("ReplicatedStorage").Remotes.Sync:InvokeServer(unpack(args))
	...
	repeat wait() until game:GetService("Players"):WaitForChild(getLocalPlayerUsername()):FindFirstChild("GameLoaded")
		...
		function checkAndSetCFrame()
			while true do
				if player:FindFirstChild("Combat") then
					if rootPart then
						rootPart.CFrame = CFrame.new(-2125, 1309, -1568)
					end
				else
					break
				end
...
while task.wait() do
    ...
    wait(1) -- Adjust the delay as needed, e.g., check every scecond
end
checkAndSetCFrame()
Hop()
]]--
